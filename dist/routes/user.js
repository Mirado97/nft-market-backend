...
router.get('/profile', authWallet_1.authenticateWallet, async (req, res) => {
    try {
        const user = await db_1.default.user.findUnique({ where: { walletAddress: req.user.walletAddress } });
        if (!user) {
...
router.post('/check-link', async (req, res) => {
    try {
        ...
        const user = await db_1.default.user.findUnique({ where: { walletAddress: walletAddress.toLowerCase() } });
